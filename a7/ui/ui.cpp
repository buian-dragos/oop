...
void Ui::user_menu(std::string user, std::string pass)
{
    ...
    std::cout << "1. CSV\n";
    std::cout << "2. HTML\n";
    std::cout << "Choose a file type to save the list: ";
    int file_type;
    std::cin >> file_type;

    UserService* userv = nullptr;

    if (file_type == 1) {
        userv = new CSV_file(eventRepo, userRepo); // Initialize with CSV_file
    } else {
        userv = new HTML_file(eventRepo, userRepo); // Initialize with HTML_file
    }
