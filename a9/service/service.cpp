...
std::vector<Event> AdminService::get_events() {
    return event_repo.get_arr();
}

bool AdminService::add_new(QString new_ev) {
    QStringList list = new_ev.split(",");
    Event new_event(list[0].toStdString(), list[1].toStdString(), list[2].toStdString(), list[3].toInt(), list[4].toStdString());
    if (event_repo.index(new_event)!=-1) {
        return false;
    }
    event_repo.add_event(new_event);
...
void UserService::open_csv() {
    std::system("start excel.exe ../repo/list.csv");
}

void UserService::save_html() {
...
std::vector<Event> UserService::get_event_list() {
    return user_repo.get_item(id).get_array();
}
